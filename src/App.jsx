...
function makeStoreIcon(color, big=false, openStatus=null) {
  const s=big?17:12, ring=openStatus!==null?s+6:0;
  const rc=openStatus===true?"#22c55e":openStatus===false?"#ef4444":"transparent";
  return L.divIcon({
    className:"",
    html: openStatus!==null
      ?`<div style="position:relative;width:${ring}px;height:${ring}px"><div style="position:absolute;inset:0;border-radius:50%;background:${rc};opacity:.35"></div><div style="position:absolute;inset:3px;border-radius:50%;background:${color};border:${big?3:2}px solid #fff;box-shadow:0 2px ${big?12:6}px rgba(0,0,0,.3)"></div></div>`
    ...
    iconSize:[openStatus!==null?ring:s,openStatus!==null?ring:s],
    iconAnchor:[openStatus!==null?ring/2:s/2,openStatus!==null?ring/2:s/2],
    popupAnchor:[0,openStatus!==null?-ring/2-2:-s/2-2],
  });
}
function makeUserIcon(){return L.divIcon({className:"",html:`<div style="position:relative;width:20px;height:20px"><div style="position:absolute;inset:0;border-radius:50%;background:#3b82f6;border:3px solid #fff;box-shadow:0 2px 10px rgba(59,130,246,.6)"></div><div style="position:absolute;inset:-7px;border-radius:50%;background:rgba(59,130,246,.15);animation:userPulse 2s infinite"></div></div>`,iconSize:[20,20],iconAnchor:[10,10],popupAnchor:[0,-14]});}
...
const IcoUser   = ()=><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>;
...
const IcoFilter = ()=><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="11" y1="18" x2="13" y2="18"/></svg>;
...
function HoursWidget({jamBuka,lang}){
  ...
  return(
    <div className="hours-widget">
      <div className="hw-today" onClick={()=>setOpen(v=>!v)}>
        <span className="hw-clock"><IcoClock/></span>
        <span className="hw-today-time">{todayDisp}</span>
        {openNow!==null&&<span className={`hw-status ${openNow?"open":"closed"}`}>{openNow?t.popup_open_now:t.popup_closed_now}</span>}
        <span className="hw-expand">{open?"▲":"▼"}</span>
      </div>
      {open&&(
      ...
      <section className="land-section">
        <div className="land-inner">
          ...
          <div className="gerai-grid">
            {[{key:"INDOMARET",img:"https://images.unsplash.com/photo-1604719312566-8912e9227c6a?w=800&q=80",fb:"https://placehold.co/800x360/fee2e2/e8404a?text=Indomaret",founded:1988,nat:"21.000+",dk:"indomaret_desc"},{key:"ALFAMART",img:"https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=800&q=80",fb:"https://placehold.co/800x360/dbeafe/2563eb?text=Alfamart",founded:1999,nat:"17.000+",dk:"alfamart_desc"},{key:"YOMART",img:"https://images.unsplash.com/photo-1583258292688-d0213dc5a3a8?w=800&q=80",fb:"https://placehold.co/800x360/ffedd5/f97316?text=Yomart",founded:2003,nat:"300+",dk:"yomart_desc"},{key:"LAWSON",img:"https://images.unsplash.com/photo-1519984388953-d2406bc725e1?w=800&q=80",fb:"https://placehold.co/800x360/cffafe/0891b2?text=Lawson",founded:2011,nat:"700+",dk:"lawson_desc"},{key:"CIRCLE K",img:"https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=800&q=80",fb:"https://placehold.co/800x360/ede9fe/7c3aed?text=Circle+K",founded:1986,nat:"300+",dk:"circlek_desc"}].map(p=>{
              const cfg=getCfg(p.key);
              const tags_id={"INDOMARET":["Swalayan","Waralaba","24 Jam","Salim Group"],"ALFAMART":["AlfaGift","Waralaba","24 Jam","Tbk Listed"],"YOMART":["Lokal Jabar","Komunitas","Terjangkau","Bandung"],"LAWSON":["Japan Style","Ready-to-Eat","Onigiri","Premium"],"CIRCLE K":["International","24 Jam","Couche-Tard","Frozone"]};
              return(
                <div key={p.key} className="gc">
                  <div className="gc-img-wrap">
                    <img src={p.img} alt={cfg.label} className="gc-img" onError={e=>{e.target.src=p.fb;}}/>
                  ...
                  <div className="gc-body">
                    <div className="gc-stats">
                      ...
                      <div><b>{p.nat}</b><span>{t.national_stores}</span></div>
                    </div>
                    <p className="gc-desc">{t[p.dk]}</p>
                    <div className="gc-tags">{tags_id[p.key]?.map((tag,i)=><span key={i} style={{background:cfg.bg,color:cfg.color}}>{tag}</span>)}</div>
                  </div>
                </div>
              );
            })}
      ...
      <section className="land-section alt-bg">
        <div className="land-inner">
          ...
          <div className="explore-cards">
            <button className="exp-card" onClick={()=>onNavigate("peta")}><div className="exp-icon" style={{background:"#f0fdf4",color:"#16a34a"}}><IcoMap/></div><div><p className="exp-title">{t.explore_map_title}</p><p className="exp-desc">{t.explore_map_desc}</p></div><span className="exp-arr">→</span></button>
            <button className="exp-card" onClick={()=>onNavigate("statistik")}><div className="exp-icon" style={{background:"#eff6ff",color:"#2563eb"}}><IcoStats/></div><div><p className="exp-title">{t.explore_stats_title}</p><p className="exp-desc">{t.explore_stats_desc}</p></div><span className="exp-arr">→</span></button>
            <button className="exp-card" onClick={()=>onNavigate("promo")}><div className="exp-icon" style={{background:"#fff1f2",color:"#e11d48"}}><IcoPromo/></div><div><p className="exp-title">{t.nav_promo || "Promo"}</p><p className="exp-desc">{lang === "id" ? "Lihat penawaran & diskon terkini" : "View latest deals & discounts"}</p></div><span className="exp-arr">→</span></button>
          </div>
        </div>
      </section>
...
function PageStats({allData,lang}){
  ...
  const hoursData=[{label:t.stats_24h,value:c24,color:"#16a34a"},{label:t.stats_limited,value:allData.length-c24,color:"#f97316"}];
  return(
    <div className="dash-page stats-page">
      <div className="page-hdr"><div><h2>{t.stats_title}</h2><p>{t.stats_sub}</p></div><span className="hdr-badge">TPBW 2024</span></div>
...
function PageMap({allData,lang}){
  ...
  const byKec={};allData.forEach(d=>{byKec[d.kecamatan]=(byKec[d.kecamatan]||0)+1;});
  ...
  const hoursOpts=[{id:"ALL",label:t.hours_all,icon:"🗂️"},{id:"open_now",label:t.hours_open_now,icon:"🟢"},{id:"24h",label:t.hours_24h,icon:"🕐"},{id:"limited",label:t.hours_limited,icon:"⏱️"}];
  ...
  const filterPanel = (
    <div className="filter-panel-inner">
      ...
      <div className="track-section">
        ...
        {!tracking?<button className="track-btn on" onClick={startTracking}><IcoTarget/>{t.track_start}</button>:<button className="track-btn off" onClick={stopTracking}>{t.track_stop}</button>}
      ...
      {nearbyList.length>0&&(
        <div className="nearby-sec">
          ...
          {nearbyList.map((d,i)=>{
            const cfg=getCfg(d.gerai);const open=isOpenNow(d.jam_buka);
            return(
      ...
      <div className="pf"><label><IcoClock/> {t.filter_hours}</label>
        <div className="hours-filter-btns">
          {hoursOpts.map(o=><button key={o.id} className={`hf-btn ${filterHours===o.id?"hf-active":""}`} onClick={()=>setFilterHours(o.id)}>{o.icon} {o.label}</button>)}
        </div>
      </div>
      <div className="pf"><label>{t.map_search}</label><input type="text" placeholder={t.map_search_ph} value={search} onChange={e=>setSearch(e.target.value)}/></div>
      <div className="pf"><label>{t.map_kec}</label>
      ...
      <div className="pf"><label>{t.map_gerai}</label>
        <div className="g-pills">
          {["ALL",...Object.keys(GERAI_CONFIG)].map(g=>{const cfg=g==="ALL"?null:getCfg(g);const active=filterGerai===g;return<button key={g} className={`pill ${active?"pill-on":""}`} style={active&&cfg?{background:cfg.color,borderColor:cfg.color,color:"#fff"}:cfg?{borderColor:cfg.color,color:cfg.color}:{}} onClick={()=>setFilterGerai(g)}>{g==="ALL"?t.map_gerai_all:getCfg(g).label}</button>;})}
        </div>
      </div>
      <div className="pf">
        <div className="radius-row"><label>{t.map_radius}</label><button className={`rad-tog ${showCircle?"on":""}`} onClick={()=>setShowCircle(v=>!v)}>{showCircle?"ON":"OFF"}</button></div>
      ...
      <div className="pf toggle-row"><label>{lang==="id"?"Ring Status Buka":"Open Status Ring"}</label><button className={`rad-tog ${showOpenRing?"on":""}`} onClick={()=>setShowOpenRing(v=>!v)}>{showOpenRing?"ON":"OFF"}</button></div>
  ...
  return(
    <div className="page-peta">
      {/* Desktop sidebar */}
      {!isMobile && <div className="map-panel">{filterPanel}</div>}

      <div className="map-wrap">
        <MapContainer center={[-6.9175,107.6191]} zoom={isMobile?11:12} style={{width:"100%",height:"100%"}} zoomControl={false}>
          ...
          {userPos&&(<>
            ...
            <Circle center={[userPos.lat,userPos.lon]} radius={userPos.acc||50} pathOptions={{color:"#3b82f6",fillColor:"#3b82f6",fillOpacity:.1,weight:1.5,opacity:.5,dashArray:"4"}}/>
          </>)}
          {markersData.map(d=>{
            const cfg=getCfg(d.gerai);const isNearest=nearbyList.length>0&&nearbyList[0].id===d.id;
            const openStatus=showOpenRing?d.openNow:null;
            return(
              <div key={d.id}>
                {showCircle&&<Circle center={[d.lat,d.lon]} radius={1000} pathOptions={{color:cfg.color,fillColor:cfg.color,fillOpacity:isNearest?opacity*2.5:opacity,weight:isNearest?2.5:1.5,opacity:isNearest?.9:.5}}/>}
                    ...
                    <div className="pop">
                      ...
                      {GERAI_WEBSITES[d.gerai]&&(
                        <div className="pop-website">
                          <a href={GERAI_WEBSITES[d.gerai].official} target="_blank" rel="noopener noreferrer" className="pop-web-btn" style={{borderColor:cfg.color,color:cfg.color}}>🌐 {GERAI_WEBSITES[d.gerai].label}</a>
                          <a href={GERAI_WEBSITES[d.gerai].promo} target="_blank" rel="noopener noreferrer" className="pop-promo-btn" style={{background:cfg.color}}>🏷️ Promo</a>
                        </div>
                      )}
        ...
        {isMobile&&(
          <button className="mobile-filter-fab" onClick={()=>setDrawerOpen(true)} aria-label="Filter">
            <IcoFilter/>
            {(filterKec!=="ALL"||filterGerai!=="ALL"||filterHours!=="ALL"||search)&&<span className="fab-dot"/>}
          </button>
        )}

        {/* Mobile: showing count overlay */}
        {isMobile&&(
          <div className="mobile-count-badge">
            {filtered.length}/{allData.length} {t.map_locations}
          </div>
        )}
      </div>
      ...
      {isMobile&&(
        <>
          ...
          <div className={`map-drawer ${drawerOpen?"drawer-open":""}`}>
            <div className="drawer-handle-wrap" onClick={()=>setDrawerOpen(v=>!v)}>
              <div className="drawer-handle"/>
            </div>
            <div className="drawer-header">
              <span className="drawer-title">{t.map_filter}</span>
              <button className="drawer-close" onClick={()=>setDrawerOpen(false)}><IcoX/></button>
            </div>
            <div className="drawer-content">{filterPanel}</div>
          </div>
        </>
      )}
    </div>
...
function Dashboard({allData,initPage,onBack,lang,setLang}){
  ...
  const t=T[lang];
  const navItems=[
    {id:"peta",      label:t.nav_map,    icon:<IcoMap/>   },
    {id:"statistik", label:t.nav_stats,  icon:<IcoStats/> },
    {id:"promo",     label:t.nav_promo,  icon:<IcoPromo/> },
    {id:"profile",   label:t.nav_profile, icon:<IcoUser/>  },
  ];
  return(
    <div className="shell">
      {/* Desktop sidebar */}
      {!isMobile&&(
        <nav className="dash-nav">
          ...
          <div className="dn-items">
            {navItems.map(n=>(
              <button key={n.id} className={`dn-item ${page===n.id?"active":""}`} onClick={()=>setPage(n.id)}>
      ...
      {isMobile&&(
        <header className="mob-topbar">
          <button className="mob-back" onClick={onBack}>← {t.nav_home}</button>
          <span className="mob-brand">🏪 MiniMap</span>
          <LangToggle lang={lang} setLang={setLang}/>
        </header>
      )}

      <div className={`dash-main ${isMobile?"dash-main-mobile":""}`}>
      ...
      <ChatBot allData={allData} lang={lang}/>
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{fetch("./data_minimarket.json").then(r=>r.json()).then(d=>{setAllData(d);setLoading(false);});},[]);

  const goTo=tab=>{setInitTab(tab);setView("dashboard");setMobileMenuOpen(false);};
  ...
  return(
    <div className="landing-shell">
      <header className="land-nav">
        ...
        {!isMobile&&(
          <nav className="ln-links">
            ...
            <button onClick={()=>goTo("promo")}>{T[lang].nav_promo || "Promo"}</button>
          </nav>
        )}
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {/* Mobile hamburger */}
          {isMobile&&(
            <button className="mob-hamburger" onClick={()=>setMobileMenuOpen(v=>!v)}>
              {mobileMenuOpen?<IcoX/>:<IcoMenu/>}
            </button>
          )}
        </div>
      </header>
      ...
      {isMobile&&mobileMenuOpen&&(
        <div className="mob-menu-dropdown">
          <button className={landTab==="home"?"mob-menu-item active":""} onClick={()=>{setLandTab("home");setMobileMenuOpen(false);}}>🏠 {T[lang].nav_home}</button>
          <button className={landTab==="profile"?"mob-menu-item active":""} onClick={()=>{setLandTab("profile");setMobileMenuOpen(false);}}>👤 {T[lang].nav_profile}</button>
          <button className="mob-menu-item" onClick={()=>goTo("peta")}>🗺️ {T[lang].nav_map}</button>
          <button className="mob-menu-item" onClick={()=>goTo("statistik")}>📊 {T[lang].nav_stats}</button>
          <button className="mob-menu-item" onClick={()=>goTo("promo")}>🏷️ {T[lang].nav_promo}</button>
        </div>
      )}
